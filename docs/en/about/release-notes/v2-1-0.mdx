...
- **[Storage backend](../../configuration/storage-backends/)**: Elasticsearch 8.x and OpenSearch 3.x are the supported storage backends. Elasticsearch uses index rollover with aliases and Index Lifecycle Management (ILM) for automatic retention and rotation, while OpenSearch uses daily time-based indices with the `jaeger-es-index-cleaner` for retention.
