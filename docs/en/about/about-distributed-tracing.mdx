...
Alauda Distributed Tracing combines Jaeger v2, Alauda Build of OpenTelemetry v2, and Elasticsearch or OpenSearch to collect, store, and visualize trace data on Kubernetes.
...
- **Alauda Distributed Tracing (Jaeger)**: This component is based on Jaeger v2 and provides the tracing backend.
    ...
    - **Jaeger ES Rollover**: This component manages rollover resources such as templates, aliases, and index rotation on Elasticsearch and OpenSearch, so long-running clusters can retain tracing data efficiently.
    - **jaeger-es-index-cleaner**: This component periodically deletes time-based indices that are older than a configured retention period. It is used by deployments that store traces in date-suffixed indices instead of rollover aliases, and is not required when a lifecycle policy governs retention.
...
- **OpenSearch**: OpenSearch is a community-driven, open source search and analytics suite that can also serve as the backend storage for Alauda Distributed Tracing. Jaeger uses the same storage implementation for OpenSearch as for Elasticsearch.
