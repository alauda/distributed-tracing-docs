...
:::tip
The new exporter is named `otlp_grpc/jaeger-v2` rather than plain `otlp_grpc` so that, if you later choose to also write to the legacy Jaeger during the observation period, the additional exporter can be patched back in symmetrically as `otlp_grpc/jaeger-v1`. See [(Optional) Enabling dual export to the legacy Jaeger](#optional-enabling-dual-export-to-the-legacy-jaeger).
:::
...
- **Faster in-place rollback.** If the new Jaeger turns out to have problems during the observation period, you can drop `otlp_grpc/jaeger-v2` from the trace pipeline with a single patch and the legacy Jaeger keeps receiving traces uninterrupted.
...
- Two export pipelines must be monitored for failures (`otelcol_exporter_send_failed_spans_total` for both `otlp_grpc/jaeger-v2` and `otlp_grpc/jaeger-v1`).

Dual export does **not** backfill pre-cutover traces into the new Jaeger; pre-cutover traces are only ever queryable through the legacy Jaeger UI.

### Enable dual export \{#enable-dual-export}

After [Switch the v2 OpenTelemetry Collector to the new Jaeger](#switch-the-v2-opentelemetry-collector-to-the-new-jaeger) has converged, the `otel` Collector has `otlp_grpc/jaeger-v2` as the only trace exporter writing to a Jaeger backend (the trace pipeline is `[debug, otlp_grpc/jaeger-v2]`). Patch the Collector to add the legacy Jaeger back as a second exporter `otlp_grpc/jaeger-v1`, mirroring the legacy-Jaeger endpoint, headless-Service load balancing, and TLS settings of the original `otlp` exporter that was removed in the switch step:
...
spec:
  config:
    ...
    service:
      pipelines:
        traces:
          exporters: [debug, otlp_grpc/jaeger-v2, otlp_grpc/jaeger-v1]
'
kubectl rollout status deployment/otel-collector -n cpaas-system --timeout=180s
```

:::warning
`service.pipelines.traces.exporters` is an array, and a merge patch replaces arrays in their entirety rather than appending. The patch above lists every exporter that must remain in the trace pipeline (`debug`, `otlp_grpc/jaeger-v2`, `otlp_grpc/jaeger-v1`). If your trace pipeline contains additional custom exporters, add them to this list before applying the patch.
:::
...
| New Jaeger has problems during the observation period (default single export) | Patch the v2 Collector to point its trace pipeline back at the legacy Jaeger collector (replace `otlp_grpc/jaeger-v2` with an exporter targeting `jaeger-prod-collector-headless.cpaas-system:4317`). The legacy Jaeger receives traces again immediately. The fastest pre-built path is to switch to dual export first via [Enable dual export](#enable-dual-export); if that path was already in use, simply patch out `otlp_grpc/jaeger-v2`. |
| New Jaeger has problems during the observation period (dual export enabled) | Patch the v2 Collector to drop `otlp_grpc/jaeger-v2` from the trace pipeline. The legacy Jaeger continues to receive traces while you investigate. |
| Legacy Jaeger has problems during the observation period (dual export enabled) | Patch the v2 Collector to drop `otlp_grpc/jaeger-v1` from the trace pipeline. This is equivalent to ending dual export early; pre-cutover traces older than the legacy retention will not be reachable in the new Jaeger. |
