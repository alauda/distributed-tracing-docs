...
2. The legacy `jaeger-es-index-cleaner` CronJob was deleted together with the legacy `Jaeger` instance, so any remaining `acp-tracing-<cluster>-jaeger-*` indices in Elasticsearch are no longer auto-rotated. Delete them manually.
    ...
    Elasticsearch defaults to `action.destructive_requires_name: true`, which rejects DELETE requests that target wildcards or `_all` with `Wildcard expressions or all indices are not allowed` (HTTP 400). The script below works around that by listing the matching indices first and then deleting each one by its explicit name.
