...
4. Watch the Collector export-failure metric to make sure the new pipeline is not silently dropping spans:
    ...
    `otelcol_exporter_send_failed_spans_total{exporter="otlp/jaeger-v2"}` should remain flat.
...
`otelcol_exporter_send_failed_spans_total` should remain flat for both `exporter="otlp/jaeger-v2"` and `exporter="otlp/jaeger-v1"`.
...
- [ ] `otelcol_exporter_send_failed_spans_total{exporter="otlp/jaeger-v2"}` does not grow continuously. (If dual export is enabled, the same applies to `exporter="otlp/jaeger-v1"`.)
