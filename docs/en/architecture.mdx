...
* **es-rollover**: Manages rollover-based index operations for Jaeger on Elasticsearch and OpenSearch. It is used to prepare aliases, indices, and templates for rollover deployments, and can periodically roll the write alias to a new index while updating read aliases.
* **es-index-cleaner**: Deletes time-based indices that are older than a configured retention period. It matches date-suffixed indices (`<prefix>-jaeger-(span|service|dependencies|sampling)-YYYY-MM-DD`) and removes those past the retention threshold. It applies to deployments that store traces in date-suffixed indices; installations that use rollover aliases with Index Lifecycle Management (ILM) on Elasticsearch or Index State Management (ISM) on OpenSearch delegate retention to the lifecycle policy instead.
