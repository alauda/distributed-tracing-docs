...
## Installing Alauda Build of Jaeger v2 \{#installing-alauda-build-of-jaeger-v2}
...
1. Create an `OpenTelemetryCollector` resource:
    ...
    ```bash {name=install-tracing:create-otel-collector}
    kubectl apply -f - <<EOF
    apiVersion: opentelemetry.io/v1beta1
    kind: OpenTelemetryCollector
    metadata:
...
2. Wait for the Collector pod to be ready:
    ...
    ```bash {name=install-tracing:wait-otel-rollout}
