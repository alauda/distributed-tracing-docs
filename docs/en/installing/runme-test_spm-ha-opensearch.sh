#!/usr/bin/env bash
# SPM 多副本（高可用）验证测试 —— OpenSearch 场景。
#
...
# 加载 SPM HA 公共验证逻辑（与 elasticsearch 版共用）
source "$(dirname "${BASH_SOURCE[0]}")/_spm-ha-common.sh"

# 测试函数：OpenSearch 场景下的 SPM 多副本验证
test_spm_ha_opensearch() {
    _spm_ha_run "opensearch"
}

# 清理函数：删除 telemetrygen 并缩容回单副本
cleanup_spm_ha_opensearch() {
    _spm_ha_cleanup
}
