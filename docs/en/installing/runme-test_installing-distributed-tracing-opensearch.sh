...
#
# 多集群:   ./run.sh --project tracing --file installing-distributed-tracing-opensearch
#           --cluster <name> 可指定安装目标集群（多集群网格的每个集群都要装一套调用链）；
#           配合 TRACING_JAEGER_ES_INDEX_PREFIX 让各集群共用一套调用链索引（步骤 5.1）；
#           注意 OpenSearch 自动安装是每个集群各建一套实例，共用索引需改为
#           TRACING_INSTALL_OPENSEARCH=false + 手动 TRACING_OPENSEARCH_* 指向同一实例。
