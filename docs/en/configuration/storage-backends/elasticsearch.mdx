...
|-------|---------|-------------|
| `max_span_age` | `72h` | Lookback applied when a read carries no explicit time range, namely trace-ID lookups and the service and operation lists. **Effective only with `rotation.periodic`**, see the warning below. |
