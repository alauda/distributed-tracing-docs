...
|-------|---------|-------------|
| `max_span_age` | `72h` | Maximum lookback for span reads. With alias-based rotation (`manual_rollover` or `auto_rollover`), set it to match the retention of the ILM policy, otherwise the Jaeger UI cannot find traces that are still stored. |
| `max_trace_duration` | `24h` | Maximum expected duration of a single trace, that is, the time between its earliest and latest span. The reader widens the time-range filter by this value on both sides, so that all spans of a trace are found even when the trace extends beyond the requested search window. |

For example, to align the lookback with an ILM policy that deletes indices after 7 days and to restrict the padding to traces no longer than one hour:
